...
st.markdown("> 자치구별 대학 수(색상)와 지하철 혼잡도(높이)를 결합하여 분석합니다.")
...
with col2:
    st.subheader("🔍 지하철 통학 인사이트")
    st.write("① **혼잡도 최상위**: 성북구, 은평구 등은 인근 대학가 역의 오전 혼잡도가 매우 높게 형성되어 있어 하차 시 병목 현상이 예상됩니다.")
    st.write("② **밀집도 주의**: 그래프 색상이 밝은 구는 대학이 밀집된 지역입니다. 혼잡도(높이)가 낮더라도 내리는 인원이 많아 실제 체감 혼잡도는 더 높을 수 있습니다.")
    st.write("③ **교차 분석**: 높이도 높고 색상도 밝은 구는 지하철 노선 증설이나 배차 간격 조정이 가장 시급한 '통학 위험 지역'입니다.")
...
with col2:
    st.subheader("🔍 인사이트")
    st.write("①대부분 출근시간인 8시에 혼잡도가 가장 높고 9시까지 점차 줄어드는 경향을 보인다. 9시부터 9시 30분까지는 각 대학별로 혼잡도가 낮아지거나 높아지므로 대학별 수치를 잘 확인하여 등교 시간을 결정해야 합니다.")
    st.write("② **동국대학교**는 8시를 제외한 모든 아침 시간에 가장 높은 혼잡도를 보입니다. 따라서 등교 시간을 최대한 늦게 가져가는 것을 추천합니다.")
