...
# ... (사용자님의 차트 3 쿼리 및 fig2 코드 생략 없이 그대로 유지)
query2 = """
SELECT 
    u."학교명", 
    s."출발역" AS "인근역", 
...
with col3_2:
    ...
    st.subheader("🔍 인사이트")
    st.write("① 8시 정점 이후 9시까지 완만해지나, 개별 수치 확인 후 등교 시간을 정하는 것이 필수적입니다.")
    st.write("② **동국대학교**는 아침 시간대 혼잡도가 높으므로 10시 이후 등교를 추천합니다.")

# --- 차트 4 ---
st.divider()
st.header("4. ⏰ 버스 등교 골든타임 분석")
# ... (사용자님의 차트 4 쿼리 및 fig_bus 코드 그대로 유지)
query3 = """
SELECT 
    '버스' AS "교통수단", 
    AVG("8시하차총승객수") AS "08시", 
...
with col4_2:
    ...
    st.subheader("🔍 인사이트")
    st.write("① **골든 타임**: 10시~14시 사이는 하차 인원이 적어 쾌적한 이동이 가능합니다.")
