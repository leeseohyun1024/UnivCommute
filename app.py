...
with col2:
    ...
    st.write("② **성북구**는 가장 많이 대학이 밀집된 지역입니다. 가장 높은 혼잡도와 더불어 가장 높은 대학교 수를 가지고 있어 체감 밀집도는 더욱 높아질 수 있습니다. 지하철 노선 중 가장 붐비는 4호선이 지나는 성북구이므로 성북구의 노선에 다니는 **지하철의 배차를 많이 편성할 필요**가 있습니다.")
...
with col2:
    st.subheader("🔍 인사이트")
    st.write("①대부분 출근시간인 8시에 혼잡도가 가장 높고 **9시까지 점차 줄어드는 경향**을 보입니다. 9시부터는 각 대학별로 혼잡도가 낮아지거나 높아지므로 대학별 수치를 잘 확인하여 등교 시간을 결정해야 합니다.")
    st.write("② **동국대학교**는 아침 시간대에 평균적으로 높은 혼잡도를 보입니다. 따라서 등교 시간을 추세가 내려가는 10시로 가져가는 것을 추천합니다.")
